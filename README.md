...
- 見出し、太字、インラインコード、Markdown表、コードブロックの変換
...
ユーザー側は `User:` / `あなた:` / `自分:`、アシスタント側は `Assistant:` / `ChatGPT:` / `AI:` に対応します。英字の大小は区別しません。区切りは半角コロンです。最初の話者マーカーより前の文章は無視されます。コードブロック内の話者マーカーは本文として保持します。Mermaidの開始フェンスは ` ```mermaid `（大文字・小文字は不問）です。Markdown表はヘッダー行の次に `| --- | --- |` のような区切り行を置いてください。JSON・ZIPのChatGPTエクスポートや、一般的なMarkdownのリスト整形には対応していません。
